...
export default function ClippedCard({ challenge }: { challenge: Challenge }) {
  ...
  return (
    <div className="relative group h-full">
      ...
      >
        <h3 className="text-3xl font-serif mb-4 leading-tight text-black capitalize">{challenge.title}</h3>
        <p className="text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-6">
          {challenge.company || "Company"} - {challenge.location || "Remote"} <br /> 
        ...
        <div className="mt-auto pt-4">
          <Link href={`/challenges/${challenge._id}`} className="font-bold text-xl text-black hover:underline inline-block pb-4">
            View Details
          </Link>
        </div>
      </div>

      {/* منطقة السهم - تم استبدال الصورة بأيقونة Lucide */}
      <div className="absolute bottom-4 right-6 w-[30%] h-[15%] flex items-center justify-end pointer-events-none">
        <div className="flex items-center gap-2 group-hover:translate-x-2 transition-transform duration-300 text-black">
          <span className="text-3xl font-light">—</span>
          {/* هنا استخدمنا الأيقونة البرمجية بدل الصورة */}
