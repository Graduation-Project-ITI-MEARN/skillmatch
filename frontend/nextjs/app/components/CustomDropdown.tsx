...
export default function CustomDropdown({
   ...
   return (
      <div className="relative w-full" ref={dropdownRef}>
         <h4 className="font-bold mb-4 uppercase tracking-widest text-xs text-gray-500">
            {label}
         </h4>
         <button
            onClick={() => setIsOpen(!isOpen)}
            className="w-full p-4 bg-gray rounded-2xl flex justify-between items-center font-bold text-sm hover:bg-gray/70 transition-colors capitalize">
            {value}
            <ChevronDown
               size={18}
