...
import { ZardButtonComponent } from 'src/app/shared/components/zard-ui/button/button.component';
import { type ZardButtonVariants } from 'src/app/shared/components/zard-ui/button/button.variants'; // <--- Import ZardButtonVariants
import { ZardCalendarComponent } from 'src/app/shared/components/zard-ui/calendar/calendar.component';
import { ZardIconComponent } from 'src/app/shared/components/zard-ui/icon/icon.component';
import {
  ZardPopoverComponent,
  ZardPopoverDirective,
} from 'src/app/shared/components/zard-ui/popover/popover.component';

import { mergeClasses } from '@/shared/utils/merge-classes';

// Now you can define your types using ZardButtonVariants properties
type ZardButtonSizeVariants = ZardButtonVariants['zSize']; // <--- Define type aliases
type ZardButtonTypeVariants = ZardButtonVariants['zType']; // <--- Define type aliases
...
export class ZardDatePickerComponent implements ControlValueAccessor {
  ...
  protected onButtonClick(event: Event): void {
    console.log('Date picker button clicked!', event);
    console.log('Popover directive:', this.popoverDirective());
    console.log('Is popover visible?', this.popoverDirective()?.['isVisible']?.());
  }

  protected readonly buttonClasses = computed(() => {
    const hasValue = !!this.value();
    const size = this.zSize();
    const height = HEIGHT_BY_SIZE[size ?? 'default'];
    return mergeClasses(
